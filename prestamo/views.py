...
def usuario_solicitar_prestamo(request):
    """
    Permite a un usuario autenticado solicitar un préstamo desde home_usuario.
    Recibe: producto[], cantidad[], fecha_devolucion_estimada, motivo.
    """
    if request.method != 'POST':
    ...
    # Crear el préstamo
    prestamo = Prestamo()
    prestamo.usuario       = str(usuario.nombre_completo)
    prestamo.nombre_usuario = str(usuario.nombre_completo)
